...
class AllPensionersPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('All Pensioners')),
      body: StreamBuilder<List<Map<String, dynamic>>>(
        ...
        builder: (context, snapshot) {
          ...
          if (pensioners == null || pensioners.isEmpty) {
            return const Center(child: Text('No pensioners found.'));
          }
                      ...
                      children: [
                        ElevatedButton(
                          onPressed: () async {
                            final idNumber = pensioner['idNumber']?.toString() ?? '';
                            if (idNumber.isEmpty) {
                              ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Cannot find ID Number')));
                              return;
                            }
                            try {
                              ...
                              await showDialog<String>(
                                ...
                                builder: (BuildContext context) => AlertDialog(
                                  ...
                                  content: const Text('Pensioner has been unblocked.'),
                                  actions: <Widget>[
                          ...
                          onPressed: () async {
                            ...
                            try {
                              ...
                              await showDialog<String>(
                                ...
                                builder: (BuildContext context) => AlertDialog(
                                  title: const Text('Alert'),
                                  content: const Text('Pensioner has been deactivated.'),
                                  actions: <Widget>[
                                    TextButton(onPressed: () => Navigator.pop(context, 'OK'), child: const Text('OK')),
                                  ],
                                ),
